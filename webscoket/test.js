    ...
    conn.on("text", async function (str) {
        ...
        if (data.privatechat == 2) {
            ...
            switch (data.type) {
                ...
                // 聊天文字信息
                case 'textsay':
                    Chat.create({ username: conn.nicname, saytext: data.text, friend: "firdensname" })
                    //广播出去
                    boardcast({
                        privatechat: 2,
                        uid: data.uid,
    ...
    if (fiesmds.length >= 1) {
        ...
        if (md5.length <= 0) {
            form.parse(req, async function (err, fields, files) {
                ...
                // 保存到数据库
                Chat.create({ username: usernamepath, saytext: 'http://127.0.0.1:3001/' + urls, fielsmd5: fiesmds[0] })
                // console.log(fiesmds[0])
                if (fielst == 0) {
                    //console.log("md5",md5)
